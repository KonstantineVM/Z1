...
class DataProcessor:
    ...
    def _process_time_series(self, df: pd.DataFrame, 
        ...
        # Ensure numeric data
        df.loc[:, time_columns] = df.loc[:, time_columns].apply(pd.to_numeric, errors='coerce')
        
        # Check for discontinuities
        def has_trailing_nan_or_zero(series):
            return pd.isna(series.iloc[-1]) or series.iloc[-1] == 0
        
        def has_leading_nan_or_zero(series):
            return pd.isna(series.iloc[0]) or series.iloc[0] == 0
        
        # Apply filters
        filter_no_trailing = df[time_columns].apply(
            lambda row: not has_trailing_nan_or_zero(row), axis=1
        )
        filter_no_leading = df[time_columns].apply(
            lambda row: not has_leading_nan_or_zero(row), axis=1
        )
        
        # Combined filter
        valid_series = filter_no_trailing & filter_no_leading
        df_filtered = df[valid_series].reset_index(drop=True)
        
        # Handle log transformation
        df_processed = self._apply_log_transformation(df_filtered, time_columns)
        
        # Transpose to have dates as index
        series_name_col = 'SERIES_NAME' if 'SERIES_NAME' in df_processed.columns else df_processed.columns[0]
        df_transposed = df_processed.set_index(series_name_col)[time_columns].T
