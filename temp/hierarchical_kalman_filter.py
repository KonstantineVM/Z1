...
class HierarchicalKalmanFilter(MLEModel):
    ...
    def transform_params(self, unconstrained):
        """Transform from optimizer space to variance space."""
        if self.transformation == 'square':
    ...
    def update(self, params, **kwargs):
        """Update state space with new parameters."""
        params = np.asarray(params)
        
        # Handle both transformed and untransformed parameters
        if 'transformed' in kwargs and not kwargs['transformed']:
            # params are already variances
            trend_variances = params[:self.n_source]
            obs_variance = params[self.n_source]
        else:
            # params need transformation
        ...
        min_variance = 1e-10
        trend_variances = np.maximum(trend_variances, min_variance)
        obs_variance = max(obs_variance, min_variance)
        
        # Update state covariance
        self['state_cov'] = np.diag(trend_variances)
        
        # Update observation covariance
        obs_cov = np.eye(self.n_series) * obs_variance
        series_idx = {name: i for i, name in enumerate(self.series_names)}
        for series in self.computed_series:
            if series in series_idx: 
                obs_cov[series_idx[series], series_idx[series]] *= self.error_variance_ratio
    ...
    def loglike(self, params, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Loglike failed with params {params}: {e}")
            return -np.inf
